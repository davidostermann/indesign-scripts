...
function exportDocument(file, firstDoc) {
  ...
  try {
     doc = app.open(file, true);
      exportOriginal(doc, ratio, fileFolder + '/original.jpg', firstDoc);
      firstDoc = false;

      // bg
      exportBackground(doc, fileFolder + '/bg.jpg', firstDoc);
...
function exportBackgrounds(doc, path, firstDoc) {
  ...
  for (var i = 0; i < count; i++) {
    ...
    if(layer.name.indexOf('fond') >= 0) {
      ...
      doc.exportFile(ExportFormat.JPG, new File(path + '/' + fileName), first);
      first = false;
      arr.push(element);
    }
  }
...
function exportOverlays(doc, ratio, path, firstDoc) {
  ...
  if (photoLayer.isValid) {
    ...
    for (var i = 0; i < photoLayer.rectangles.length; i++) {
      ...
      //photoLayer.rectangles[i].images[0].exportFile(ExportFormat.JPG , new File(path+i+'.png'), true);
  ...
  if (graphicsLayer.isValid) {
    ...
    for (var i = 0; i < graphicsLayer.groups.count(); i++) {
      ...
      if(group.isValid) {
        ...
        newDoc.exportFile(ExportFormat.JPG, new File(fullFileName), true);
...
function exportOriginal(doc, ratio, fullName, firstDoc) {
  ...
  doc.exportFile(ExportFormat.JPG, new File(fullName), firstDoc);
}
